...
export const notesRouter = tsr.router(notesContract, {
  list: async () => {
    const notes = db.query("SELECT * FROM notes ORDER BY created_at DESC").all() as Note[];
  ...
  get: async ({ params }) => {
    const note = db.query("SELECT * FROM notes WHERE id = ?").get(params.id) as Note | null;
    if (!note) return { status: 404 as const, body: { message: "Note not found" } };
